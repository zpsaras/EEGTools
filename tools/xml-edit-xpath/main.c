 ...
 * usage:	xpath2 <xml-file> <xpath-expr> <new-value> <out-file>
 * test:	xpath2 test3.xml '//discarded' discarded > xpath2.tmp && diff xpath2.tmp $(srcdir)/xpath2.res
 * author: 	Aleksey Sanin and Daniel Veillard with the slightest of modifications by Zach Psaras
...
static int example4(const char *filename, const xmlChar * xpathExpr,
                    const xmlChar * value, const char* output);
...
main(int argc, char **argv) {
    ...
    /* Do the main job */
    if (example4(argv[1], BAD_CAST argv[2], BAD_CAST argv[3], argv[4])) {
	usage(argv[0]);
	return(-1);
    }
...
usage(const char *name) {
    ...
    fprintf(stderr, "Usage: %s <xml-file> <xpath-expr> <new-value> <output-file>\n", name);
}
 ...
 */
static int 
example4(const char* filename, const xmlChar* xpathExpr, const xmlChar* value, const char* output) {
    xmlDocPtr doc;
